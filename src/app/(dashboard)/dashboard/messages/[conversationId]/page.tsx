...
export async function generateMetadata({
  ...
  const conv = await prisma.conversation.findUnique({
    ...
    select: { listing: { select: { streetAddress: true } } },
  });
  return {
    title: conv
...
export default async function ConversationPage({
  ...
  // Verify the conversation exists and the user is a party to it
  ...
  if (!target) notFound();
  if (target.buyerId !== user.id && target.sellerId !== user.id) {
    redirect("/dashboard/messages");
  }

  // Fetch all conversations for the sidebar (same as the list page)
